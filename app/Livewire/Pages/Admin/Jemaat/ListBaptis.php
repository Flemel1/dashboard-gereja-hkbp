...
class ListBaptis extends Component
{
    private function getDataForDataTable()
    {
        // Fetch all complaints from the database
        $jemaats = Baptis::select(['id', 'jemaat_id', 'nama_jemaat', 'created_at'])
            ->with(['jemaat'])
        ...
        $formattedData = $jemaats->map(function ($jemaat) {
            ...
            $btnEdit = '<a href="' . route('admin.baptis.edit', $jemaat->id) . '">
    ...
    public function delete($id)
    {
        try {
            ...
            redirect()->route('admin.baptis.list');
        } catch (Exception $ex) {
    ...
    public function render()
    {
        ...
        return view('livewire.pages.admin.jemaat.list-baptis', compact('config'));
    }
}
