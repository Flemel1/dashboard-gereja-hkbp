<div class="card">
    <div class="card-header">Edit Kematian</div>
    <div class="card-body">
        <form wire:submit.prevent="update">
            <x-adminlte-select wire:model.lazy="jemaat_id" label="Nama Jemaat (Optional)" name="jemaat_id"
                ...
                @foreach ($jemaats as $id => $name)
                    ...
                    @endif
                    <option value="{{ $id }}" @if($id == $jemaat_id) selected @endif>{{ $name }}</option>
                @endforeach

            </x-adminlte-select>

            <x-adminlte-input wire:model.lazy="tanggal_kematian" label="Tanggal Kematian" name="tanggal_kematian"
                type="date" error-key='tanggal_kematian' placeholder="Pilih Tanggal Kematian" />
    ...
    @script
        <script>
            $wire.on('kematian-updated', (event) => {
                const {
                    title,
                    message
                } = event[0];
